...
//    }
    ...
    public void start() {
        ...
        for (int i = 1; i <= Parameters.agentCount; i++) {
            Int2D pos = this.generateRandomAgentLocation();
            createAgent(SimpleTWAgent.createAgent(i, pos.getX(), pos.getY(), this, Parameters.defaultFuelLevel));
        }
...
//        
    ...
    private Int2D generateRandomAgentLocation() {
        int gx = 1, gy = 1;
        while (!isValidCreationLocation(
                gx = this.random.nextInt(this.xDimension),
                gy = this.random.nextInt(this.yDimension)) || this.agentGrid.get(gx, gy) != null) {
        }
